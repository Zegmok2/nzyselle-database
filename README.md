...
| Credential storage abstraction (`core/src/credentials.rs`) | ✅ Trait + in-memory test double fully tested. The real `WindowsCredentialStore` backend has now also been confirmed working against the real Windows Credential Manager in an actual user session (2026-08-08) — a developer-app Client ID/Secret and a real OAuth token pair were both saved and read back successfully |
...
| Real TikTok/Instagram/YouTube adapters (`core/src/{tiktok,instagram,youtube}_adapter.rs`) | ⚠️ Written against each platform's current public API docs, compiles clean, and has been through several rounds of code-review bug fixes (see `src/lib/changelog.ts`). **YouTube's OAuth connect flow is now confirmed working against a live Google account** (2026-08-08) — the first of the three to clear that bar. Publishing a video through YouTube, and everything about TikTok/Instagram (connect and publish alike), remains unverified against a live account |
