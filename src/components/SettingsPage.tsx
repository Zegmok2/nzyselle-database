...
import { PrimaryButton, SecondaryButton } from "./dashboard/Toolbar";
import { APP_VERSION, CHANGELOG, type ChangeKind } from "../lib/changelog";
import { isRunningInTauri } from "../lib/tauriBackend";

const CHANGE_KIND_LABEL: Record<ChangeKind, { text: string; color: string }> = {
  feature: { text: "Feature", color: "var(--accent)" },
  fix: { text: "Fix", color: "var(--warning)" },
  optimization: { text: "Optimization", color: "var(--success)" },
};
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {state.phase === "up-to-date" && <span style={{ fontSize: 12, color: "var(--success)" }}>You're on the latest version.</span>}
        {state.phase === "error" && <span style={{ fontSize: 12, color: "var(--error)" }}>{state.message}</span>}
      </div>
      ...
      {state.phase === "downloading" && <span style={{ fontSize: 12, color: "var(--text-secondary)" }}>Downloading update…</span>}
      {state.phase === "ready" && <span style={{ fontSize: 12, color: "var(--success)" }}>Installed. Restarting…</span>}
    </div>
  );
}
...
export function SettingsPage({ workspace, onUpdated }: { workspace: Workspace; onUpdated: (w: Workspace) => void }) {
  ...
  return (
    <div style={{ padding: 32, maxWidth: "var(--content-max-width)", margin: "0 auto", display: "flex", flexDirection: "column", gap: 32 }}>
      <div>
        <h1 style={{ margin: 0, fontSize: 20, fontWeight: 600, letterSpacing: "-0.01em" }}>Workspace Settings</h1>
      </div>
      ...
      <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <label style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 13 }}>
          Accent color
          <input type="color" value={accentColor} onChange={(e) => setAccentColor(e.target.value)} style={{ ...inputStyle, height: 36, padding: 2 }} />
        </label>
        <label style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 13 }}>
          Default watch folder
          <input value={defaultWatchFolder} onChange={(e) => setDefaultWatchFolder(e.target.value)} placeholder="C:\path\to\folder" style={inputStyle} />
        </label>
        <div>
      ...
      <section>
        <h2 style={{ fontSize: 15, marginBottom: 10 }}>Platform Developer Apps</h2>
        ...
        {credentialError && <div style={{ color: "var(--error)", fontSize: 12, marginBottom: 8 }}>{credentialError}</div>}
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {(["tiktok", "instagram", "youtube"] as const).map((platformId) => (
            <div key={platformId} style={{ padding: 12, background: "var(--panel-strong)", borderRadius: "var(--radius-sm)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, fontSize: 13, fontWeight: 600, textTransform: "capitalize" }}>
                {platformId}
                {configuredCredentials.includes(platformId) && (
                  <span style={{ fontSize: 11, fontWeight: 400, color: "var(--success)" }}>Configured</span>
                )}
              </div>
              <form onSubmit={(e) => handleSaveCredentials(platformId, e)} style={{ display: "flex", gap: 8 }}>
                ...
                <input
                  ...
                  style={inputStyle}
                />
                <SecondaryButton disabled={credentialSaving === platformId}>
                  {credentialSaving === platformId ? "Saving…" : "Save"}
                </SecondaryButton>
              </form>
            </div>
          ))}
      ...
      <section>
        <h2 style={{ fontSize: 15, marginBottom: 10 }}>Backup &amp; Restore</h2>
        <p style={{ color: "var(--text-secondary)", fontSize: 12, marginBottom: 12 }}>
        ...
        <form onSubmit={handleBackup} style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          <input value={backupPath} onChange={(e) => setBackupPath(e.target.value)} placeholder="Destination file path, e.g. C:\backups\nzyselle-2026-08-07.sqlite" style={inputStyle} />
          <SecondaryButton disabled={backupBusy || !backupPath.trim()}>Back up now</SecondaryButton>
        </form>

        <form onSubmit={handleRestore} style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          <input value={restorePath} onChange={(e) => setRestorePath(e.target.value)} placeholder="Backup file path to restore from" style={inputStyle} />
          <SecondaryButton disabled={backupBusy || !restorePath.trim()}>Restore</SecondaryButton>
        </form>

        {backupError && <div style={{ color: "var(--error)", fontSize: 12 }}>{backupError}</div>}
        {restoreMessage && <div style={{ color: "var(--success)", fontSize: 12 }}>{restoreMessage}</div>}

        {backups.length > 0 && (
          <ul style={{ listStyle: "none", padding: 0, marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
            {backups.map((b) => (
              <li key={b.id} style={{ fontSize: 12, color: "var(--text-secondary)" }}>
      ...
      <section>
        <h2 style={{ fontSize: 15, marginBottom: 4 }}>About</h2>
        ...
        >
          ...
          <span>
            Only the built-in Sandbox platform has been connected and posted to end-to-end. The real
            TikTok, Instagram, and YouTube adapters are implemented against each platform&rsquo;s public
            API docs but have not yet been run against a live account — check the status shown on each
            platform&rsquo;s card on the Connections page before relying on it.
          </span>
        </div>
          ...
          {CHANGELOG.map((entry) => (
            <div key={entry.version}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 600 }}>v{entry.version}</span>
                  ...
                  >
                    <span
                      style={{
                        ...
                        fontSize: 10,
                        fontWeight: 600,
                        color: CHANGE_KIND_LABEL[c.kind].color,
                        border: `1px solid ${CHANGE_KIND_LABEL[c.kind].color}`,
                        borderRadius: 4,
                        padding: "2px 6px",
                        marginTop: 1,
                        whiteSpace: "nowrap",
